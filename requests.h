...
void sendRequest(esphome::esp32_can::ESP32Can* can_bus)
{
    ...
    ESP_LOGI("main", "request(%d, %d)", request_index++, data);
}
...
{
    ESP_LOGI("onHandle", "can_id<%s> data<%s>", to_hex(can_id).c_str(), to_hex(data).c_str());
    switch (can_id)
    {
    case 0x180:
        ...
        if (data[2] == 0xFA and data[3] == 0x06 and data[4] == 0x9B) {
            ...
            ESP_LOGI("main", "Temperature received over can is %f", temperature);
        }
        ...
        if (data[2] == 0xFA and data[3] == 0xC0 and data[4] == 0xFF) {
            ...
            ESP_LOGI("main", "Aussentemperatur: %f", temperature);
        }
