...
public:
    ...
    std::array<uint8_t, 7> const& data() const
    {
        return m_data;
    }
    ...
    void send(esphome::esp32_can::ESP32Can* pCanBus) {
        ...
        ESP_LOG_FILTER("main", "request(%d, %s, %s)", m_request_index, getName().c_str(), to_hex(m_data).c_str());
    ...
    bool inProgress() const {
        return m_last_request > m_last_update;
    }
private:
...
public:
    ...
    void handle(esphome::esp32_can::ESP32Can* pCanBus, uint32_t can_id, std::vector<uint8_t> const& responseData, uint32_t timestamp) {
        ESP_LOG_FILTER("handle", "can_id<%s> responseData<%s>", to_hex(can_id).c_str(), to_hex(responseData).c_str());
        TRequest& request = findRequest(can_id, responseData);
        request.handle(can_id, responseData, timestamp);

        sendNextPendingRequest(pCanBus);
    }
...
private:
    TRequest& findRequest(uint32_t can_id, std::vector<uint8_t> const& responseData) {
        ...
        return m_requests[0];   // should never happen!
    }
    ...
    TRequest* getNextRequestToSend() {
        ...
        for (auto& request : m_requests) {
            if ((timestamp > request.getLastUpdate() + m_update_interval) && !request.inProgress()) {
                return &request;
            }
        }
        return nullptr;
